...
% S_sweep = linspace(1500,2600,12);
% AR_sweep = linspace(6,12,25);
% S_sweep = linspace(1500,2600,23);
% % input_sweep = linspace(1.8,3,13);
% % input_sweep = linspace(2.8,3,2);
% input_sweep = linspace(1,2,11);

% AR_sweep = linspace(6,12,7);
...
%         v20k = (-speedofsound(0)*0.25+speedofsound(10000)*0.35)/10000;
%         mission.v_cruise = speedofsound(0)*0.25 + v20k*(h);
        idle_thrust_percent = 0.1-h/10000*0.099;
    end
    if h > 10000 && h < 25000
%         hdot = 1500/60; %fps
...
%     HexDrag = interp3D_V003(FCmap_drag,1,Pgraph(counter-1)/weight.fuelcell,mission.M,mission.altitude,'n')*weight.fuelcell;
    ...
    D = 1/2*rho*mission.v_cruise^2*ac.wing.S*(CD) + HexDrag;
    [Tout,Pout] = power2thrust_v01(mission.M,ac,h);
    Tr = idle_thrust_percent*interp1(Pout,Tout,FCmaxpower(weight,h,mission.M)*FC2shaft(h));

%     Tr = D - hdot*(state.W3-Fuelburn)/mission.v_cruise;
    hdot = -mission.v_cruise*(Tr-D)/(state.W3-Fuelburn);
    if Pgraph(counter-1)/weight.fuelcell < 260
        FCin = 259;
    else
        FCin = Pgraph(counter-1)/weight.fuelcell;
    end
    FCeff = interp3D_V003(FCmap_eff,1,FCin,mission.M,mission.altitude,'n');
%     FCeff = interp3D_V003(FCmap_eff,1,Pgraph(counter-1)/weight.fuelcell,mission.M,mission.altitude,'n');
    Fuelburn = Fuelburn + H2CT(Tr,FCin*weight.fuelcell,FCeff)*dt*Tr;
    h = h - hdot*dt;
    if h < 0
        h = 0;
    end
...
%     Tr = D - hdot*(state.W7-Fuelburn)/mission.v_cruise;
    [Tout,Pout] = power2thrust_v01(mission.M,ac,h);
    Tr1 = idle_thrust_percent*interp1(Pout,Tout,FCmaxpower(weight,h,mission.M)*FC2shaft(h));
...
%     if Pgraph(counter-1)/weight.fuelcell < 301
%         FCin = 300
%     else
        FCin = Pgraph(counter-1)/weight.fuelcell;
%     end
