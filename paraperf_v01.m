...
length_of_run = length(AR_sweep)*length(S_sweep)*length(input_sweep);
...
for i = 1:length_of_run
    RefMat(i,1) = input_sweep(i_ct);
    RefMat(i,2) = S_sweep(S_ct);
    RefMat(i,3) = AR_sweep(AR_ct);
    
    i_ct = i_ct+1;
    if i_ct > length(input_sweep)
        i_ct = 1; 
        S_ct = S_ct+1;
...
outputfilename2 = ['output_comparison_par.xlsx'];
