...
N_sweep = 30;
min_iterations = 5;
summarytable_out = array2table(zeros(1,34));
AR_count = 0;
S_count = 1;
input_count = 1;
is_borked = 0;
...
% AR_sweep = 135.7^2/2160;
% S_sweep = 2160;
S_sweep = linspace(1500,2500,11);
% input_sweep = linspace(0.7,1,7);
input_sweep = 1;
file_out = 0;
% input_sweep = 1;
...
% press_ratio = 1;
Takeoff_Parameter = (state.W0/ac.wing.S)/(press_ratio*CLto*max(Tgraph)/state.W0);
...
% if TOFL > max_TOFL
%     ToP_required = interp1(RunwayLength,TakeoffParam,max_TOFL);
%     TO_Thrust = (state.W0/ac.wing.S)/(1*CLto*ToP_required)*state.W0;
%     ac.size.max_power = TO_Thrust*vgraph(1)*1.3558/(0.7*0.95*0.995); 
%     Tgraph(1) = TO_Thrust;
%     TOFL = max_TOFL;
% end
...
h = cruise_alt;
% prop_eff = FanEff(h);
% thrust_to_powerplant_eff = 0.785;  %from ansell
% FCeff = FCefficiency(h,ac.size.sizing_power,weight);
% cruise_scaling = prop_eff*thrust_to_powerplant_eff*FCeff*.95*1030/1328;
cruise_scaling = (FanEff(h)*0.449)/(FanEff(0)*0.579)*1030/1328;

% if dataout.TA_TO_test > dataout.TR_TO
%     dataout.TA_cruise_test = dataout.TA_TO_test*cruise_scaling; %Sized by climb
% else
%     dataout.TA_cruise_test = dataout.TA_V35_test*cruise_scaling; %sized by t/o
% end
% 
% if dataout.TA_cruise_test > dataout.TR_cruise
%     dataout.TA_cruise = data.TA_cruise_test; %sized by TO or V35
% else
%     dataout.TA_cruise = dataout.TR_cruise; %sized by cruise
% end
...
if dataout.TA_TO_test_fc > dataout.TR_TO
    ...
    is_scaled_by_cruise = 1;
    dataout.sizedby = 37;
    Takeoff_Parameter = (state.W0/ac.wing.S)/(1*CLto*dataout.TA_TO/state.W0);
    TOFL = interp1(TakeoffParam,RunwayLength,Takeoff_Parameter);
end

if dataout.TA_V35_test_fc > dataout.TR_V35
    % V35 is sized by cruise
    sprintf('Sized by cruise! \n')
    dataout.TA_TO = dataout.TA_TO_test_fc;
    dataout.TA_V35 = dataout.TA_V35_test_fc;
    dataout.TA_cruise = dataout.TR_cruise;
...
%Now test TO and V35, skip if scaled by cruise
if is_scaled_by_cruise == 0

    dataout.TA_V35_test_TO = dataout.TR_TO*static_scaling;
    dataout.TA_TO_test_V35 = dataout.TR_V35/static_scaling;
    
    if dataout.TA_V35_test_TO > dataout.TR_V35
        % scaled by TO
        sprintf('Sized by TO! \n')
        dataout.TA_V35 = dataout.TA_V35_test_TO;
        dataout.TA_TO = dataout.TR_TO;
        dataout.TA_cruise = dataout.TR_TO*static_scaling*cruise_scaling;
        dataout.sizedby = 0;
        Takeoff_Parameter = (state.W0/ac.wing.S)/(1*CLto*dataout.TA_TO/state.W0);
        ac.size.max_power = dataout.TA_TO*vgraph(1)*1.3558/(0.7*0.95*0.995);
        TOFL = interp1(TakeoffParam,RunwayLength,Takeoff_Parameter);
    ...
    if dataout.TA_TO_test_V35 > dataout.TR_TO
        % scaled by V35
        sprintf('Sized by V35! \n')
        dataout.TA_V35 = dataout.TR_V35;
        dataout.TA_TO = dataout.TA_V35/static_scaling;
        dataout.TA_cruise = dataout.TA_TO*static_scaling*cruise_scaling;
        dataout.sizedby = 1;
        Takeoff_Parameter = (state.W0/ac.wing.S)/(1*CLto*dataout.TA_TO/state.W0);
        ac.size.max_power = dataout.TA_TO*vgraph(1)*1.3558/(0.7*0.95*0.995);
        TOFL = interp1(TakeoffParam,RunwayLength,Takeoff_Parameter);
...
outputtable = table(tgraph',Tgraph',vgraph',Mgraph',hgraph',hdotgraph',CDgraph',CLgraph',CDograph',CDigraph',CDwgraph',CDowinggraph',CDofusegraph', Cfgraph',Regraph', Wgraph', PPgraph', MPgraph', Pgraph');
outputtable.Properties.VariableNames = {'Time, min.' 'Thrust, lbf' 'VTAS, fps' 'Mach No.' 'Altitude, ft' 'Climb Rate, Feet per Second' 'Total Aircraft CD' 'Total Aircraft CL' 'Profile Drag' 'Induced Drag' 'Wave Drag' 'Wing Profile Drag' 'Fuselage Profile Drag' 'Fuselage Skin Friction' 'Reynolds Number' 'Weight, Lbs' 'Thrust Power, Watts' 'Motor Power, Watts' 'Fuel Cell Power, Watts'};
