...
N_sweep = 30;
min_iterations = 8;
summarytable_out = array2table(zeros(1,13));
AR_count = 0;
S_count = 1;
input_count = 1;
...
AR_sweep = 1;
S_sweep = 1;
input_sweep = 35000; %Payload Sweep

%Initial Count
file_out = 0;
for outer_loop = 1:length(input_sweep) % ! UPDATE
...
summarytable = table(input_sweep(input_count),dataout.n,weight.Wpay,ac.WF,state.W0,LRC_range,max(Tgraph),TOFL,MTOW_LFL,climb_distance/6076,state.t2,max(CLgraph),is_borked);
...
% end
summarytable_out.Properties.VariableNames = {'Input','Ref. File','Payload Weight','Fuel Weight','MTOW','Cruise Range','Max Thrust','TOFL','Maximum Landing Field Length','Climb Distance','Time to Climb','Maximum CL during Flight','Errored Out'};
