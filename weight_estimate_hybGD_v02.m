...
function [ac,weight] = weight_estimate_hybGD_v02(ac,weight)
...
ac.WMZF = WMZF;
WE_final = 0;
% weight.wing1 = 0.0017*ac.WMZF*(wing.b/cosd(wing.sweep_at_1_2))^.75*(1+sqrt(6.3*cosd(wing.sweep_at_1_2)/wing.b))*ac.nult^0.55*(wing.b*wing.Sref/(wing.tr*ac.WMZF*cosd(wing.sweep_at_1_2)))^0.3;
weight.wing2 = (0.00428*ac.wing.S^.48*ac.wing.AR*ac.MH^0.43*(WTO*ac.nult)^.84*ac.wing.taper_ratio^.14)/((100*ac.wing.tcavg)^.76*cosd(ac.wing.c1_2_sweep)^1.54);

% weight.wing = weight.wing*(1 + 0.5*.463110019);
weight.wing1 = 0.0017*WMZF*(ac.wing.bref/cosd(ac.wing.c1_2_sweep))^0.75*(1+(6.3*cosd(ac.wing.c1_2_sweep)/ac.wing.bref)^0.5)*ac.fuse.nult^0.55*(ac.wing.bref*ac.wing.S/(ac.wing.tr*WMZF*cosd(ac.wing.c1_2_sweep)))^0.3*ac.wing.weight.has_spoilers*ac.wing.weight.has_engines*ac.wing.weight.has_lndgear*ac.wing.weight.is_braced;
weight.wing = (weight.wing1+weight.wing2)/2;
