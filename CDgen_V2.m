function [REsweep, Msweep, CLsweep, CDout, CDoout, CDiout, CDwout] = CDgen_V2(ac,N_sweep)

REsweep = linspace(1e7,4e7,N_sweep);
Msweep = linspace(0.2,0.8,N_sweep);
CLsweep = linspace(0.02,1.25,N_sweep);
...
%     Rwf = Rwf_find(M,RNHT);
    Rwf = 1;
    RLS = RLS_find(M,ac.HT.ct_cmax_sweep);
%     Cfw = .455/(log10(RNHT))^2.58/(1+.144*M^2)^.65; %save as
%     maing wing, above
...
%     Cfw = .455/(log10(RNVT))^2.58/(1+.144*M^2)^.65;
