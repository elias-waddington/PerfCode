...
takeoff_type = 3; %power = 1, thrust = 0, hybrid = 2, 3 = time step
if takeoff_type == 0
...
elseif takeoff_type == 3
    TOFL_error = 1;
%     T = 30000; % Initial Guess
%     [Fnet_lbf,Pshaft_W] = thrust2power(0);
%     ShaftPower = weight.fuelcell*interp2(MP_h_rng,MP_M_rng,MP_Wlb_mat,0,.25)*FC2fan(0)/FanEff(0);
%     %%% CURRENT WORKING SPOT
%     while ShaftError > 100
%         
% %         ShaftError = Pshaft_W - ShaftPower;
%         ShaftError = (Pshaft_W - ShaftPower)/ShaftPower
%     end
...
% ac.size.max_power = ac.size.sizing_power*FC2fan(0)/(FanEff(0) * 0.95); %adjust sizing power to motor output
    ...
    if FC_TO_size > FC_C_size
        ...
        dataout.TA_cruise = (FC_TO_size*interp2(MP_h_rng,MP_M_rng,MP_Wlb_mat,37000,.773)-dataout.HotelP)/(speedofsound(37000)*0.773 /FC2fan(37000)*1.3558)
    ...
    elseif FC_C_size > FC_TO_size
        ...
        Vstall = sqrt(state.W0*2/(rho*ac.wing.S*CLto));
        Vrotate = Vstall*1.08;
        P_shaft = weight.fuelcell*interp2(MP_h_rng,MP_M_rng,MP_Wlb_mat,0,.25)*FC2fan(0)/FanEff(0)/.95;
        %acceleration phase. DT = 1 second
        while V(ii) < Vrotate
            qbar = 0.5*V(ii)^2*mission.rho;
            HexDrag = HexDrag_coeff*qbar;
