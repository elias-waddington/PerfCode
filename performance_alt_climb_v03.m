...
input_sweep = linspace(1.8,2.6,9);

% AR_sweep = linspace(7,12,3);
% S_sweep = linspace(1600,2500,3);
...
ac.WF = 16000; %initial fuel guess
...
elseif takeoff_type == 2
%     
%     %double check net vs gross thrust
%     ToP_required = interp1(RunwayLength,TakeoffParam,max_TOFL);
%     HexDrag = interp2(MP_h_rng,MP_M_rng,MP_Drag_mat,0,0.25)*weight.fuelcell;
%     dataout.TR_TO = (state.W0/ac.wing.S)/(1*CLto*ToP_required)*state.W0;
%     dataout.TO_TotalThrust = dataout.TR_TO*static_scaling + HexDrag;
%     P_shaft_guess = 3e7;
%     t_error = 1;
%     while t_error > 0.005
%         [Fnet,dataout.TA_TO] = thrust2powerv02(0.25,P_shaft_guess);
%         t_error = (-Fnet+dataout.TO_TotalThrust)/dataout.TO_TotalThrust;
%         P_shaft_guess = P_shaft_guess + P_shaft_guess*t_error*.6;
%     end
%     dataout.TO_power = P_shaft_guess/(FC2fan(0)/FanEff(0))+ HotelLoads(0.25,0,1);
%     %replace above line with solver per dave's work
%     FC_TO_size = dataout.TO_power/interp2(MP_h_rng,MP_M_rng,MP_Wlb_mat,0,0.25);
%     
%     ac.size.sizing_power = FC_TO_size*interp2(MP_h_rng,MP_M_rng,MP_Wlb_mat,0,0.25);
%     ac.size.max_power = ac.size.sizing_power*(FC2fan(0)/FanEff(0));
% 
%     TOFL = max_TOFL;
%     %check for TOC scaling
%     h = 37000;
...
%     dataout.sizedby = 0;
% 
%     if FC_C_size > FC_TO_size
%         dataout.sizedby = 36;
%         disp('SIZED BY TOC')
% 
%         ac.size.sizing_power = FC_C_size*interp2(MP_h_rng,MP_M_rng,MP_Wlb_mat,0,0.25);
%         ac.size.max_power = ac.size.sizing_power*(FC2fan(0)/FanEff(0));
%         [dataout.TO_TA,dataout.TR_TO] = thrust2powerv02(0,ac.size.max_power);
%         Takeoff_Parameter = (state.W0/ac.wing.S)/(press_ratio*CLto*dataout.TO_TA/static_scaling/state.W0);
%         TOFL = interp1(TakeoffParam,RunwayLength,Takeoff_Parameter);
%     end
%     
...
%     [TOFL,dataout.TA_TO,dataout.TR_TO] = takeoff_solver(weight,Msweep,REsweep,CLsweep,CDout,MP_h_rng,MP_M_rng,MP_Drag_mat,MP_Wlb_mat,state,ac,CLto);
    dataout.TR_TO = 0;
    ac.size.sizing_power = FC_C_size*interp2(MP_h_rng,MP_M_rng,MP_Wlb_mat,0,.25); %Adjust to sea level power output
    ac.size.max_power = ac.size.sizing_power*FC2fan(0)/(FanEff(0) * 0.95); %adjust sizing power to motor output
    dataout.sizedby = 36;
    weight.fuelcell = FC_C_size;
    [dataout.TA_TO,FPR] = thrust2powerv02(0,FC_C_size*interp2(MP_h_rng,MP_M_rng,MP_Wlb_mat,0,.25)*FC2fan(0)/FanEff(0)-HotelLoads(0.25,0,1));
    
%     if ~isnan(dataout.TA_TO)
...
%         TOFL = max_TOFL*1.2;
%     end
if isnan(dataout.TA_TO) | isnan(FPR) | ~isreal(dataout.TA_TO) | ~isreal(FPR)
    is_borked = 1;
    disp('BORK due to TO conditions')
    TOFL = max_TOFL*1.2;
    dataout.TA_TO=0;
    dataout.TR_TO=0;
    dataout.TA_V35=0;
    ...
    if TOFL > max_TOFL
        ...
        weight.fuelcell = FC_TO_size;
        ac.size.sizing_power = FC_TO_size*interp2(MP_h_rng,MP_M_rng,MP_Wlb_mat,0,.25)+ HotelLoads(0.25,0,1);
        ac.size.max_power = ac.size.sizing_power*FC2fan(0)/(FanEff(0) * 0.95); %adjust sizing power to motor output
...
%     dataout.TA_TO = ac.size.sizing_power
    ...
    if TOFL > max_TOFL
        iii = 1;
        dataout.sizedby = 0;
        while abs(TOFL_error) > 0.01
            [TOFL,dataout.TA_TO,dataout.TR_TO] = takeoff_solver(weight,Msweep,REsweep,CLsweep,CDout,MP_h_rng,MP_M_rng,MP_Drag_mat,MP_Wlb_mat,state,ac,CLto);
            TOFL_error = ((TOFL-8200)/8200);
            delta2 = weight.fuelcell*TOFL_error*0.35;
            weight.fuelcell = weight.fuelcell + delta2;
%             state.W0 = state.W0 + delta2;
            iii = iii + 1;
            if iii > 50
                break
            end
...
outputtable = table(tgraph',vgraph',Mgraph',hgraph',hdotgraph',CDgraph',CLgraph',CDograph',CDigraph',CDwgraph',CDowinggraph',CDofusegraph', Cfgraph',Regraph', Wgraph', Pagraph',Prgraph',Tagraph',Trgraph', Tgraph',PPgraph', MPgraph', Pgraph');
outputtable.Properties.VariableNames = {'Time, min.' 'VTAS, fps' 'Mach No.' 'Altitude, ft' 'Climb Rate, Feet per Minute' 'Total Aircraft CD' 'Total Aircraft CL' 'Profile Drag' 'Induced Drag' 'Wave Drag' 'Wing Profile Drag' 'Fuselage Profile Drag' 'Fuselage Skin Friction' 'Reynolds Number' 'Weight, Lbs' 'FC Power Available, Watts' 'FC Power Required, Watts' 'Thrust Available, Lb' 'Thrust Required, Lb' 'Thrust Commanded, Lb' 'Thrust Power, Watts' 'Motor Power, Watts' 'Fuel Cell Power, Watts'};
