...
sweep14_sweep = 22.422; %*linspace(.9,1.1,11);%*linspace(.8,1.2,21);
sweep12_sweep =  18.761; %*linspace(.9,1.1,11);%*linspace(.8,1.2,21);
sweepmct_sweep =  20.686; %*linspace(.9,1.1,11);%*linspace(.8,1.2,21);
taper_sweep = .2861; %.*linspace(.9,1.1,11); %*linspace(.8,1.2,21); %[0.9*.2861,.2861,1.1*.2861];
tc_sweep = .09458; %*linspace(.9,1.1,11);
incid_sweep = 0; %linspace(-5,5,11); %this is only the root section
% h_sweep = 1.5*linspace(.9,1.1,11);
h_sweep = 1.5;
% add dihedral
...
%     end
    ...
    TSFCgraph(counter) = H2CT(Tr,mission.v_cruise,h,weight,FCeff);
    if battery_use == 1
...
counter = counter-1;
TSFCgraph = TSFCgraph(1:counter).*3600;
...
outputfilename2 = ['output_comparison_par5.xlsx'];
