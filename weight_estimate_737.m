...
function [ac,weight] = weight_estimate_737(ac,weight)
...
WTO = 173000; %this will have to be a guess i suppose
...
WE_final = WE_final+weight.emp.HT;
weight.emp.VT = ac.VT.kh*ac.VT.S*(3.81*(ac.VT.S^0.2*ac.VD)/(1000*sqrt(cosd(ac.VT.c1_2_sweep)))-0.287);
WE_final = WE_final+weight.emp.VT;
...
weight.propulsors = 10432; %lbs. Fixed per Dave Hall estimate
...
weight.fuelsys = 80*(2+3-1)+15*3^0.5*(ac.WF/6.55)^0.333
