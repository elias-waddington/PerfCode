...
dave_numbers = 1; %toggles profile, CF drag terms
drag_breakout = 1; %toggles o, i, w drag terms (1 is on, 0 is off)
...
% input_sweep = linspace(1.6,2.6,11);
...
dataout.toc_test = (100/60)*state.W0/mission.v_cruise + CD*qbar*ac.wing.S;
...
ac.size.sizing_power = dataout.TR_TO * static_scaling * speedofsound(0)*0.25 /FC2fan(0)*1.3558 ;
...
if dataout.toc_to > dataout.TR_TO
    ...
    ac.size.sizing_power = dataout.toc_to * speedofsound(0)*0.25/FC2fan(0) *1.3558*static_scaling; %must be in power required at take-off amounts
    ac.size.max_power = calcMotorPower(dataout.TR_V35,speedofsound(0)*0.25,0,weight);
